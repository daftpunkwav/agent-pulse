...
| **安全默认** | release 模式强制非默认密码、API Key 鉴权、OTLP 认证已启用 |
...
> **当前阶段 (v0.1.x) 安全基线**: 已实现 X-AgentPulse-Key 头鉴权（配置文件白名单 + SHA-256 比对），OTLP 接收端（HTTP + gRPC）加 body size 限制与 Key 校验，DSN 密码不在日志中明文输出（`MaskedDSN()` 脱敏），release 模式启动时强制非默认密码校验，速率限制（Token Bucket，per-IP）已实现，CORS 白名单配置。

| 场景 | 当前实现 | 未来改进 |
|------|---------|---------|
| API 鉴权 | `X-AgentPulse-Key` 头（配置文件白名单，SHA-256 恒定时间比对）+ Rate Limit（per-IP Token Bucket） | DB/API Key CRUD + JWT/OAuth2 |
| OTLP 接入 | `X-AgentPulse-Key` + `MaxBytesReader`（默认 10MB）+ Read/Write Timeout（HTTP + gRPC 均已实现） | mTLS / IP 白名单 |
| 数据库密码 | `MaskedDSN()` 输出脱敏 + release 模式强制非默认密码 | Kubernetes Secrets 集成 |
| PII 数据 | input/output preview 截断 + evaluate 路径 regex 脱敏 + `capture_args=False` 默认 | LLM 自动检测 + 完整脱敏流水线 |
| CORS | `AllowedOrigins` 白名单，凭证场景动态回显 | 动态域名管理 |
...
> **OTLP 接收**: HTTP/protobuf (端口 4318) + gRPC (端口 4317) 均已实现,`AGENTPULSE_OTLP_HTTP_PORT` / `AGENTPULSE_OTLP_GRPC_PORT` 配置端口。
