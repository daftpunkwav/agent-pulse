...
| Go SDK | OpenTelemetry Go SDK + OTLP HTTP/gRPC Exporter | Phase 3 计划中，当前推荐直接用 OTLP |
| Python SDK | OpenTelemetry Python SDK + LangChain/LangGraph 适配器 | 兼容前三个项目 |
| 前端 | **Next.js 15 + Recharts + D3.js + Tailwind CSS v4** | 与其他项目保持一致 |
...
├── web/                # Next.js 前端（Trace 查看 / 成本归因 / 评估 Dashboard）
├── sdk-python/         # Python SDK（LangChain/LangGraph/AutoGen 适配器）
├── sdk-go/             # Go SDK（Phase 3 计划中，当前用 OTLP 直推）
├── docs/               # PRD/架构/API/SDK 文档
├── deploy/             # Dockerfile + K8s manifests + 初始化 SQL
├── .env.example
├── .gitignore
└── README.md
```
...
- `deploy/` 下分 `docker/`（镜像）、`k8s/`（K8s 清单）、`init/`（数据库初始化）
